...
export class Transcript {
  constructor(id, data, strand, geneName, geneId) {
    this.id = id;
  ...
  getUtrCoordinates() {
    this.fivePrimeUtrStart =
      this.strand === "+" ? this.exons[0][0] : this.exons[0][1];
    this.threePrimeUtrEnd =
      this.strand === "+"
        ? this.exons[this.exons.length - 1][1]
        : this.exons[this.exons.length - 1][0];
  ...
  isIn5Utr(position) {
    ...
    if (
      this.strand === "+" &&
      position >= this.fivePrimeUtrStart &&
      position <= this.fivePrimeUtrEnd
    ) {
      return true;
    } else if (
      position >= this.fivePrimeUtrEnd &&
  ...
  isIn3Utr(position) {
    ...
    if (
      this.strand === "+" &&
      position >= this.threePrimeUtrStart &&
      position <= this.threePrimeUtrEnd
    ) {
      return true;
    } else if (
      position >= this.threePrimeUtrEnd &&
  ...
  getExonSeqs() {
    ...
    for (var i = 0; i < this.exons.length; i++) {
      ...
      this.exonSeqs.push(
        this.cdnaSeq.slice(runningSum, runningSum + exonLength)
      );
    ...
    if (this.isProteinCoding) {
      for (var i = 0; i < this.cds.length; i++) {
        ...
        for (var j = 0; j < this.exons.length; j++) {
          ...
          var end =
            (this.strand == "+" ? cds[1] - exon[0] : exon[1] - cds[0]) + 4;
  ...
  getLengths() {
    this.cdnaLength = this.exons
      .map((exon) => exon[1] - exon[0] + 1)
      .reduce((a, b) => a + b);

    if (this.cds.length !== 0) {
      this.cdsLength = this.cds
        .map((cds_i) => cds_i[1] - cds_i[0] + 1)
        .reduce((a, b) => a + b);
